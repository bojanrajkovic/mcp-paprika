...
0. **Start from current `main`, and re-sync before you push.** `git fetch` and rebase your work onto `origin/main` before grounding yourself below, so the steps that follow are reasoning about the latest tree, not a stale one. Over a longer change, check mergeability again right before opening the PR: if `main` moved, rebase onto it and re-verify (typecheck/test/lint) on the new base rather than opening a stale branch. A refactor that lands while you work can invalidate the plan or collide with the exact surface you are editing (a tree-wide rename, a moved or renamed module); when it does, the intent usually survives untouched and only its application to the new code changes — re-apply on the new base rather than fighting three-way conflict markers.
