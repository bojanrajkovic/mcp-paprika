...
## Extension: resources as data (2026-06-25)

Resources adopt the same `{ spec, register }` shape: `defineResource(spec, ctx => ({ read, list? }))` returns a `ResourceDef`, and the kernel registers each via `resource.register(ctx)` exactly as it does tools (`src/kernel/resource.ts`). The motivation differs from tools — there the win was boot-free doc-gen; here it is **structural tracing**. Resources used to register as opaque `(ctx) => void` closures, so the `tracedResourceRead` wrap (the `resources/read` span + `mcp.server.operation.duration`) was applied by hand in each file and held honest only by a grep-shaped conformance test that scanned `src/domains/*/resources/` — which silently missed the `widgets` resource under `src/features/`. Moving the wrap into `defineResource`'s `register` makes it impossible to register a resource untraced (the grep gate is deleted), and adds the `resource read` invoke log resources previously lacked. The spec being data also lets the doc generator grow a resources section the same way it reads tool specs — deferred, not built.

`ResourceSpec` is `{ primary, aliases?, kind? }` rather than the flat `{ name, uriTemplate, description }` the surface first suggested, because the photo resource registers **two** URI templates (a bare `…/photo` and a catch-all `…/photo{+rest}`) sharing **one** read handler and one telemetry kind: `primary` is the single listable template, `aliases` are read-only URIs routed to the same `read`, and `kind` (defaulting to `primary.name`) lets both photo registration names record under one telemetry label. The rejected alternative was a flat `templates[]` array with a per-template `list` flag (every template a co-equal peer): rejected because it makes list-binding conventional (a `list:true` flag in the spec bound by name to the factory's `list` impl, mis-flaggable with no compile error) where `primary` + a single optional `list?` makes it type-enforced, and it charges the four single-template resources an array-of-one plus a mandatory `kind`. The "exactly one template lists, the rest are read-only" invariant holds for all five resource families today; a future family with two co-equal listing templates would be the trigger to revisit.

## References

- Issue [#289](https://github.com/bojanrajkovic/mcp-paprika/issues/289) — the resource-symmetry extension (structural resource tracing).
