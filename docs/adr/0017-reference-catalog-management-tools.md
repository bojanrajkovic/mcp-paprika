...
The grocery/pantry renderers resolve an item's aisle display name through the live catalog (`ctx.deps.aisle.get(item.aisleUid)?.name`), falling back to the item's denormalized copy only for a dangling/no-aisle reference. This is the same pattern recipes already use for category names (FK-only, resolve at render — see `src/cache/CLAUDE.md`), and it makes a rename one catalog write. The denormalized name on the wire copies of old items goes stale; nothing this server renders reads it anymore.
...
- **`delete_meal_type` lives in the meal-planner coordinator** (which sees meal + menu for the impact counts, and the catalog write via `MealTypeApi.deleteMealType`). It **warns-and-proceeds**: the response reports how many meals/menu items referenced the type, the renderers already skip dangling `typeUid`s, and blocking on append-only meal history would make any type ever used in a logged meal permanently undeletable. Built-ins are deletable; auto-create resurrects the name on next reference (as a custom type), so an accidental delete is recoverable.
...
- `delete_meal_type` can orphan meal/menu references by design; anything new that renders a `typeUid` must follow the existing skip-dangling convention.
