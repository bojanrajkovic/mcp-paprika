...
1. logs `tool invoked` (uniform `{ tool }` shape, info) **before** the gate, so a gated call is still visible — plus the `args` on a separate **debug** line, so per-call correlation (which UID, which list) is recoverable by raising the level. The args record is **sanitized**: any string past 256 chars becomes a length marker (a ~13 MB base64 image or a full recipe's directions never lands in a retained log), URL-shaped strings are stripped to protocol + host + path with the query collapsed to a marker (credentials embed _inside_ url strings — userinfo, presigned-URL signatures — where key-based redaction can't see), and credential-named fields (`generation_token`, …) are censored by the root logger's `REDACT_PATHS`;
