...
- **The construction-time dependency graph is wide and shallow.** All 12 in-memory stores take a single `pendingWriteTtlMs` scalar and depend on nothing else. The only fan-in nodes are `SyncEngine` (reads the stores plus client, cache, and log; `src/paprika/sync.ts` takes the whole `AppContext`), the vector store (recipe + category stores plus `sync.events`), and the auth context (the disk cache). Every cross-store relationship is a runtime lookup through the context, not a constructor dependency.
