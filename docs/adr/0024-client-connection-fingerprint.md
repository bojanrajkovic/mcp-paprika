...
**Measure forwarding with a config-gated active probe.** A diagnostics flag registered a single tool that returned a fresh random token in `structuredContent` only, never in the text — the deliberate inverse of the both-channels rule. Asking the model in a host to read the token back revealed whether that host forwarded the structured channel to the model. The probe was **conditionally registered** (in the module's tool list only when the flag was on), so it was absent from the advertised surface in production, not merely inert — and excluded from the generated tool reference for the same reason. Identity (the fingerprint) and behavior (the probe result) are maintained together as a host → forwarding matrix; the dual-channel result of ADR-0023 remains the unconditional fail-safe regardless of what the matrix shows.

> **Probe lifecycle (removed after the spike).** The probe was a temporary measurement instrument, not a product feature. It populated the host → forwarding matrix during the #358 spike (Claude Code / Desktop / mobile, ChatGPT, mcp-cli — see the matrix doc) and was then **removed from the code** along with its `MCP_DIAG` flag; re-add it to re-verify a host after a client update. The **fingerprint telemetry above is the durable, shipped half** of this ADR. The key finding the probe established: a host's UI/apps capability is orthogonal to whether it forwards `structuredContent` to the model, so the dual-channel floor (ADR-0023) stays.
