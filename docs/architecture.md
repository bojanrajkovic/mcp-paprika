...
The rollout is staged (ADR-0019): the meal reads (`read_meal_plan`, `search_meal_history`, `read_recipe_history`) are the first adopters — their rendered text omits meal UIDs, so without the structured channel the model cannot drive `reschedule_meal` / `delete_meal` / `update_meal` after a read. See [ADR-0019](adr/0019-mcp-app-widget-surface.md).
