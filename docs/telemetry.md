...
- Under HTTP, request SERVER spans from the `@hono/otel` middleware (probe paths and the long-lived `GET /mcp` SSE stream excluded); tool spans parent under them. Each `POST /mcp` is a **new root** — `OTEL_PROPAGATORS=none` disables inbound `traceparent` adoption, because the MCP clients (ChatGPT, Claude) send trace context but export to their own backends, so joining their trace would leave every trace rooted in a span this collector never receives. On top of `@hono/otel`'s method/route/status/`url.full`, each request span carries the caller fingerprint it omits: `user_agent.original`, `client.address`, and (when it differs) the socket `network.peer.address`, plus an `origin` / `accept-language` request-header allowlist. `client.address` is the **real** client IP only when `MCP_TRUST_PROXY=true` — it then reads `x-forwarded-for` (leftmost) / `cf-connecting-ip` set by the trusted front proxy; without it, only the socket peer is trusted (a directly-exposed client can forge those headers), the same trust model the DCR rate limiter uses. The allowlist deliberately excludes `authorization`/`cookie` (credentials) and `referer` (a URL whose query the export-path `url-scrub` can't reach inside a header value).
