...
**Behavior is not in the handshake.** The capabilities tell you what a client _advertises_, not whether it forwards `structuredContent` to its model — the MCP capability set has no such flag. That behavior column is filled by the `MCP_DIAG`-gated `diag_forwarding_probe` tool (a structured-only token the model reads back) and maintained as an identity→behavior matrix; see `docs/configuration.md` for the flag.
