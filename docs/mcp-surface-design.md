...
The Content / Data / Reference classification heuristic that decides whether each Paprika entity is exposed as a tool, a resource, or both — and the reasoning behind it (notably why a `read_X` tool is not redundant with the `paprika://X/{uid}` resource) — now lives in ADR-0004, which is canonical.
...
This file is retained only as a stable link target for the design plans that cite it. New work should reference ADR-0004 instead.
