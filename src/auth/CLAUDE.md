...
- **Bounded memory.** All three in-memory stores (`AuthRequestStore`, `AuthCodeStore`, `PendingAuthorizationStore`) are capped at `MAX_INMEMORY_AUTH_ENTRIES` (50). `/authorize` is unauthenticated and unthrottled, so an attacker who registers a client could otherwise flood these maps; the cap bounds the live set, and `AuthCleanup` (6h sweep) plus `put`'s sweep-on-full reclaim expired entries. The two unauthenticated write paths — `redirectUpstream` (AuthRequest, recognized origin) and `authorize`'s consent branch (Pending, unrecognized origin) — refuse with **503** when full rather than evict an in-flight auth or send a user to the IdP with no state to return to. `AuthCodeStore`'s `put` is reached only after a completed upstream login + allowlist pass, so its cap is defense-in-depth (degrades to `invalid_grant` if ever hit, not a flood surface).
