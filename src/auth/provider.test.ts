...
describe("MintingOAuthServerProvider", () => {
  ...
  describe("authorize", () => {
    it("redirects to upstream /authorize with state, nonce, scope, and our client_id", async () => {
      const ctx = {
  ...
  describe("exchangeAuthorizationCode", () => {
    it("valid auth_code → returns access+refresh+expires_in=86400+token_type=Bearer", async () => {
      // Set up a complete exchange. Put an AuthCodeState into authCodes via `authCodes.put(code, state)`, then call `provider.exchangeAuthorizationCode(client, code, codeVerifier, redirectUri, resource)`. Assert the returned `OAuthTokens` has `access_token` (starts with `mcp_at_` or whatever prefix tokens.ts uses), `refresh_token`, `token_type === "Bearer"`, `expires_in === ACCESS_TOKEN_TTL_SECONDS` (24h = 86400), and `scope` equal to the AuthCodeState scope.
    ...
    it("consumed auth_code → InvalidGrantError on second exchange", async () => {
      // Exchange once successfully, then call again with the same code. Second call must reject with `InvalidGrantError` (assert `.rejects.toMatchObject({ errorCode: "invalid_grant" })`).
    ...
    it("resource mismatch → invalid_target error", async () => {
      // Stored AuthCodeState has resource="https://m.example.com/mcp"; call exchange with `resource: new URL("https://wrong/")`. Reject with `errorCode: "invalid_target"`.
  ...
  describe("verifyAccessToken", () => {
    ...
    it("throws InvalidTokenError for unknown/expired token", async () => {
      // Call with random string. Reject with `errorCode: "invalid_token"`.
  ...
  describe("revokeToken", () => {
    ...
    it("idempotent — revoking unknown token returns void without throw", async () => {
      // Call with a random token. Resolves to undefined without throwing.
      await expect(
  ...
  describe("challengeForAuthorizationCode", () => {
    it("returns the code_challenge stored at /authorize time", async () => {
      // Put an AuthCodeState with `codeChallenge: "abc"`. Call `provider.challengeForAuthorizationCode(client, code)`. Resolves to "abc".
    ...
    it("does NOT consume the auth_code (subsequent exchangeAuthorizationCode succeeds)", async () => {
      // Call challenge, then call exchange with same code. Both succeed (the code is only consumed by `exchangeAuthorizationCode`, not by `challengeForAuthorizationCode`'s peek).
    ...
    it("unknown code → InvalidGrantError", async () => {
      // Call with random code. Reject with `errorCode: "invalid_grant"`.
  ...
  describe("logging", () => {
    ...
    it("emits info record on authorization_code grant token mint", async () => {
    ...
    it("no revocation log on no-op revoke (unknown token)", async () => {
      await logProvider.revokeToken(mockClient, {
