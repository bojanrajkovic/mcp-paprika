...
- **Export only the real contract; keep private sub-shapes file-local.** `module.ts` exports the intra-domain ctx contracts its own co-located tools/syncs import (`<Domain>State`, `<Domain>Writes`) but NOT the private helper shapes that compose them — the `*EntitySlice` store+cache pairs are `interface`s with no `export`, used only to build `*State` in the same file. A schema's inferred type lives beside its schema, not in `api.ts`: the shared `MealTypeSpec` (`z.infer<typeof mealTypeSpecSchema>`) sits in `meal-type-helpers.ts` next to the schema, so the sites that annotate a spec value (`resolveOrCreateMealType`'s and `resolveSpec`'s params, `log_cooked_meal`'s default) name `MealTypeSpec` instead of re-spelling `z.infer<typeof …>`. (Tools that only pass the schema's parsed `args.type` straight through don't annotate it, so they keep importing just the schema.)
