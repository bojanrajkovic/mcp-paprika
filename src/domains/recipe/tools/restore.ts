...
import type { ErrorResult } from "../../../shared/tools.js";
    ...
    return async (args) => {
      ...
      const recipe = await (
        ...
        async (e): Promise<ErrorResult> => {
          if (e instanceof PaprikaAPIError && e.status === 404) {
            ...
            return errorResult(`No recipe found with UID "${args.uid}" (it may not exist or was already deleted).`);
          }
          // Transient/upstream failure — don't masquerade as "already active".
          log.error({ err: e, uid: args.uid }, "restore_recipe lookup failed");
          return errorResult(`Failed to look up recipe "${args.uid}": ${e.message}`);
        },
      );
      ...
      const saved = (await ctx.infra.client.saveRecipe(updated)).match(
        ...
        (e) => {
          ...
          return errorResult(`Failed to restore recipe: ${e.message}`);
        },
      );
