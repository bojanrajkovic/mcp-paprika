 ...
 * list's purchased items, writing through `ctx.state.commitGroceryItemsBatch`.
 */
      ...
      return groceryStartGuard(ctx.state).match(
        async (): Promise<CallToolResult> => {
          ...
          try {
            ...
            await ctx.state.commitGroceryItemsBatch(saved);
          } catch (error) {
 ...
 * list, writing through `ctx.state.commitGroceryItemsBatch`.
 */
