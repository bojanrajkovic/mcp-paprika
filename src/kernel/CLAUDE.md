...
- **Sync is a dumb driver over per-module reconciles, and recipe must lead.** `buildKernel`'s `syncOnce` runs `core`-tier reconciles first in dependency order (a throw aborts the cycle), then `additive` ones best-effort, then flush + sweep; the whole cycle never throws. Recipe is hoisted first by a stable sort on the id `"recipe"` (the legacy engine marked the recipe store synced before the others, and the dependency DAG can't express that ordering). A cycle reports its `AnySyncResult[]` ONLY after reaching flush — an aborted/un-flushed cycle returns `[]`, so a partial cycle fans out no resource notification.
