...
- `MealType` — Meal type catalog entry with 7 fields (`uid`, `name`, `color`, `orderFlag`, `originalType`, `exportAllDay`, `exportTime`); output of `MealTypeStoredSchema` and `MealTypeSchema`. No `deleted` field on the type — but POST bodies carry `deleted: true` to soft-delete (the wire format is the same shape; only `deleted` flips). `exportTime` is `number` (seconds since midnight: `28800` for 08:00, `64800` for 18:00). `originalType` is `number | null` — built-in types carry the integer mapping to one of the four defaults (Breakfast=0, Lunch=1, Dinner=2, Snacks=3); user-created custom types carry `null`. Not used by the read-only history feature.
