...
- `recipe-hash.ts` — `computeRecipeHash(recipe)`: local reimplementation of Paprika's client-owned recipe content hash (#167), reverse-engineered from the macOS app's `Recipe.hashValues`. Used by `client.ts`'s write chokepoint so writes carry the correct `hash`. Parity is pinned to real framework output by `src/paprika/__fixtures__/recipe-hashes.ts` (regenerate via `scripts/recipe-hash-fixtures/run.sh`).
...
- `saveRecipe(recipe: Readonly<Recipe>): Promise<Recipe>` — stamps the content hash (see `stampContentHash` below), serializes recipe to camelCase-to-snake_case JSON, gzip-compresses, POSTs as `FormData` with `data.gz` attachment, and **returns the hash-stamped recipe** (callers commit the return so the cache matches what was POSTed)
- `private stampContentHash(recipe): Recipe` — the single write-side hash chokepoint (#167). Only the hard-delete tombstone (`deleted: true`) echoes the existing hash verbatim — Paprika validates `deleted` against the stored hash server-side (#125). Everything else recomputes via `computeRecipeHash`, **including a soft-delete / `inTrash` toggle**: the hash is trash-independent, so a pure trash flip recomputes to the same content hash (no-op for a current recipe) while a content edit that also sets `inTrash: true` (e.g. `update_recipe` renaming + trashing at once) still gets a fresh, detectable hash. Soft-deletes are not hash-validated, so recomputing is safe. Called by `saveRecipe` and `uploadPhoto`, which return the stamped recipe so the wire payload and the locally-committed recipe carry the same hash.
...
- `uploadPhoto(recipeWithPhoto, photo, thumbnail, full): Promise<Recipe>` — attaches a photo to a recipe via the verified 3-request sequence (recipe POST with thumbnail → photo POST with full image → recipe re-POST). The caller (`upload_photo` tool) supplies the recipe with its `photo`/`photoLarge`/`photoHash` fields already set, the 7-field Photo entity, and the two normalized JPEG buffers. Stamps the recipe's content hash (a photo attach changes hashed fields) via `stampContentHash` and **returns the hash-stamped recipe** so `attachPhotoToRecipe` commits a hash-consistent recipe. See the photo upload wire format below.
...
- `postEntities<T>(url: string, items: ReadonlyArray<Readonly<T>>, toPayload: (item: Readonly<T>) => Record<string, unknown>): Promise<void>` — maps `items` through `toPayload`, passes the resulting array to `buildEntityFormData`, then calls `request("POST", url, z.boolean(), formData)`. Used by all seven non-recipe save/delete methods (`saveCategory`, `deleteCategory`, `saveAisle`, `savePantryItems`, `saveGroceryList`, `saveGroceryItems`, `saveGroceryIngredient`)
...
The recipe `hash` is **not** validated on a photo attach (the probe attached with an empty hash successfully) — only the `deleted` tombstone path validates the hash (#125). The recipe's content hash is nonetheless stamped on the photo POST (`uploadPhoto` → `stampContentHash`, #167) so a photo attach doesn't leave the recipe looking "changed" on the next sync. The Photo entity `hash` is `sha256` of the bytes WE upload (Paprika stores client hashes verbatim). Images are normalized to JPEG via `sharp` in `tools/photo-helpers.ts` (the client takes prepared buffers).
...
- Eleven stores' `sweepPending()` runs every cycle (store, categoryStore, pantryStore, aisleStore, groceryListStore, groceryItemStore, mealStore, mealTypeStore, menuStore, menuItemStore, photoStore). `groceryIngredientStore` is NOT swept (no pending-writes). Observation-based clearing handles upserts; TTL sweep is the only clearing mechanism for pending-deletes.
- No `sync:complete` event is emitted for the ingredient catalog, meal types, meals, or photos (reference/non-resource entities)

**Dependencies:**

- **Uses:** `AppContext` (client, cache, store, categoryStore, pantryStore, aisleStore, groceryListStore, groceryItemStore, groceryIngredientStore, mealStore, mealTypeStore — `server` and `notifier` are intentionally absent; notifier is used via subscriber pattern only), `mitt` (event emitter), `node:timers/promises` (scheduler.wait), `./types.js` (Recipe, RecipeUid, Meal, GroceryList, GroceryItem, AnySyncResult, RecipeSyncResult, PantrySyncResult, GroceryListSyncResult, GroceryItemSyncResult, DiffResult)
