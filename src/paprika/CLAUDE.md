...
- **`syncOnce()` never throws and never calls the notifier directly.** Every error is caught, logged, and emitted as `sync:error`; the loop continues. Resource-list notification is the job of a `sync:complete` subscriber wired in `buildAppContext`, not the engine — which is why `SyncEngine` takes `AppContext` (process-wide) and the meal/menu/photo blocks are each isolated in their own best-effort `try/catch` so an additive read surface can't abort core sync.
