...
- `MealType` — Meal type catalog entry with 7 fields (`uid`, `name`, `color`, `orderFlag`, `originalType`, `exportAllDay`, `exportTime`); output of `MealTypeStoredSchema` and `MealTypeSchema`. No `deleted` field. `exportTime` is `number` — seconds since midnight (e.g. `28800` for 08:00, `64800` for 18:00). Not used by the read-only history feature.
