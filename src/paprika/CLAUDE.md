...
- `private stampContentHash(recipe): Recipe` — the single write-side hash chokepoint (#167). A live recipe gets a freshly computed `computeRecipeHash`; a trash/hard-delete tombstone (`inTrash || deleted`) echoes the existing hash verbatim (#125: the empty-trash tombstone is validated against the stored hash server-side, and a soft-delete's hash already matches the server). Called by `saveRecipe` and `uploadPhoto`, which return the stamped recipe so the wire payload and the locally-committed recipe carry the same hash.
