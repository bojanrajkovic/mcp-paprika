...
- `MealType` — Meal type catalog entry with 7 fields (`uid`, `name`, `color`, `orderFlag`, `originalType`, `exportAllDay`, `exportTime`); output of `MealTypeStoredSchema` and `MealTypeSchema`. No `deleted` field. `exportTime` is `string | number` — current production always returns seconds-since-midnight integers (`28800` for 08:00), but the union accepts the ISO time-string format (`"08:00:00"`) observed in older HAR captures as a defensive shim. Not used by the read-only history feature.
