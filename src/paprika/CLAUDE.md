...
- **Sync is never-throws and never calls the notifier directly.** The kernel's `syncOnce` driver catches every error and continues; resource-list notification is the interval loop's job — `notifyFromResults` (`src/server/sync-loop.ts`) fires off the `AnySyncResult[]` a completed cycle returns, not the reconcile. Each domain's reconcile (`syncReplaceAllEntity` for replace-all entities, the bespoke diff-and-fetch for recipes) propagates its errors to the driver, which runs `reference` catalog reconciles (aisle/category/meal-type) first and best-effort, then `core` ones (a throw aborts the cycle), then isolates `additive` ones (meals/menus/photos) in their own best-effort handling — so neither a reference catalog nor an additive read surface can abort core sync (see [ADR-0010](../../docs/adr/0010-reference-sync-tier.md)). (`syncReplaceAllEntity` itself throws on a fetch failure, and re-enters that throw-abort contract from cache `Result`s via `unwrapSyncStep` — the one interim bridge, removed when #264 makes the driver Result-aware; it's the driver that swallows.)
