...
Per-session builder. Constructs a fresh `McpServer`, wraps `app` into a `SessionContext` by adding the server reference, and registers all 46 unconditional tools plus the recipe, grocery-list, and menu resource families. The 3 new category CRUD tools (`registerCreateCategoryTool`, `registerUpdateCategoryTool`, `registerDeleteCategoryTool`) are always registered. `registerDiscoverTool` is registered only when `app.vectorStore !== null` (semantic search is opt-in via config).
