...
Per-session builder. Constructs a fresh `McpServer`, wraps `app` into a `SessionContext` by adding the server reference, and registers all 35 tools plus the recipe, grocery-list, and menu resource families. `registerDiscoverTool` is registered only when `app.vectorStore !== null` (semantic search is opt-in via config).
...
- **Uses:** `@modelcontextprotocol/sdk` (`McpServer`, `LoggingMessageNotification`), `../paprika/` (`PaprikaClient`, `SyncEngine`), `../cache/` (`RecipeStore`, `PantryStore`, `AisleStore`, `GroceryListStore`, `GroceryItemStore`, `GroceryIngredientStore`, `MealStore`, `MealTypeStore`, `MenuStore`, `MenuItemStore`) and `../cache/disk/` (`DiskCacheRoot`), `../features/` (`VectorStore`, `buildDiscoverComponents`), `../tools/` (all `register*Tool` functions), `../resources/` (`registerRecipeResources`, `registerGroceryListResources`, `registerMenuResources`), `../utils/` (`PaprikaConfig`, `getCacheDir`), `../auth/` (`buildAuthContext`)
