...
| `.restore(token, entry)`      | `(string, GeneratedImageEntry) => void` — re-insert a consumed entry under its ORIGINAL token (preserves `createdAt`); used to give a token back when an attach fails after an atomic `consume` |
...
- Single-use, race-safe: `consume` deletes before returning. `upload_photo` `consume`s the token atomically up front (the synchronous delete runs before any `await`, so two concurrent calls can't both attach the same preview), then `restore`s it if validation or the attach fails — so a wrong-recipe or transient-upload failure leaves the preview spendable for a retry without ever allowing a double-attach.
