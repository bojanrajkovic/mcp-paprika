...
| `.peek(token)`                | `(string) => GeneratedImageEntry \| null` — read WITHOUT removing (lazily evicts if expired); validate a token while leaving it spendable |
...
- Single-use: `consume` deletes before returning; a token attaches at most once. `upload_photo` `peek`s to validate/use a token and only `consume`s after a successful attach, so a wrong-recipe or transient-upload failure leaves the preview spendable for a retry.
