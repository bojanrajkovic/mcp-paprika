...
describe("generate_recipe_photo tool", () => {
  ...
  it("generates with the default model from a name-based prompt, attaches, and reports cost + UID", async () => {
    ...
    // The attach path echoes the recipe + new photo UID over structuredContent, with
    // no generationToken (presence-discriminated from the preview path).
    const structured = result.structuredContent as { recipeUid: string; photoUid: string; generationToken?: string };
    expect(structured.recipeUid).toBe(RECIPE_UID);
    expect(structured.photoUid.length).toBeGreaterThan(0);
    expect(structured.generationToken).toBeUndefined();
  });
  ...
  it("attach:false stashes the full generated bytes under a returned token", async () => {
    ...
    // The preview path echoes the recipe + the SAME generation token over structuredContent.
    expect(result.structuredContent).toEqual({ recipeUid: RECIPE_UID, generationToken: token });
