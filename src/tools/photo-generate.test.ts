...
function setup(opts?: {
  ...
  return { callTool, generate, uploadPhoto, getRecipe, ctx };
}
...
describe("generate_recipe_photo", () => {
  it("generates with the default model from a name-based prompt, attaches, and reports cost + UID", async () => {
    ...
    const result = await callTool("generate_recipe_photo", { recipe_uid: RECIPE_UID });
  ...
  it("appends the style hint to the prompt", async () => {
    ...
    await callTool("generate_recipe_photo", { recipe_uid: RECIPE_UID, style: "on white marble" });
    expect(lastOptions(generate).prompt).toContain("on white marble");
  });

  it("forwards the chosen model and aspect_ratio", async () => {
    const { callTool, generate } = setup();
    await callTool("generate_recipe_photo", { recipe_uid: RECIPE_UID, model: "gpt-image", aspect_ratio: "4:3" });
    const opts = lastOptions(generate);
  ...
  it("attach:false returns an inline ~280px thumbnail preview without uploading", async () => {
    ...
    const result = await callTool("generate_recipe_photo", { recipe_uid: RECIPE_UID, attach: false });
  ...
  it("attach:false stashes the full generated bytes under a returned token", async () => {
    ...
    const stashed = ctx.generatedImageStore.consume(token!);
  ...
  it("reports a not-found for an unknown recipe", async () => {
    ...
    const result = await callTool("generate_recipe_photo", { recipe_uid: RecipeUidSchema.parse("nope") });
  ...
  it("restyle_existing errors when the recipe has no photo", async () => {
    ...
    const result = await callTool("generate_recipe_photo", { recipe_uid: RECIPE_UID, restyle_existing: true });
  ...
  it("restyle_existing re-fetches the authoritative recipe and passes its photo as a reference image", async () => {
    ...
    await callTool("generate_recipe_photo", { recipe_uid: RECIPE_UID, restyle_existing: true });
