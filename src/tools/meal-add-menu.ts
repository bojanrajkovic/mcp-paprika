      ...
      return coldStartGuard(ctx)
        ...
        .match(
          async (): Promise<CallToolResult> => {
            ...
            const menuItems = ctx.menuItemStore.getByMenuUid(menu.uid);
            ...
            for (const mt of ctx.mealTypeStore.getAll()) typeByUid.set(mt.uid, mt);
            ...
            try {
              const saved = await ctx.client.saveMeals(builtItems);
              await commitMealsBatch(ctx, saved);
            } catch (error) {
              const message = toMessage(error);
              log.error({ err: error, uid: menu.uid, count: builtItems.length }, "saveMeals failed");
              return textResult(`Failed to add menu to planner: ${message}`);
            }

            return textResult(renderPlannerAdds(menu.name, startDay, materialized));
          },
          (guard) => guard,
        );
    },
