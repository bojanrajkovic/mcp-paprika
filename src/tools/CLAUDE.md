...
- **Boundary: tool handlers reach data only through `ctx`.** Access state via `ctx.store` / `ctx.client` / the typed stores on `SessionContext` — never import `paprika/` or `cache/` _runtime_ modules. Runtime imports of Zod schemas from `paprika/types.js` (input validation) and anything in `utils/` are fine; `import type` from `paprika/` and `cache/` is fine. Branded `XxxUidSchema` input fields are type-only at runtime, so a parsed `args.uid` flows into `ctx.store.get(...)` with no re-parse/cast at the lookup; remaining `XxxUidSchema.parse(...)` calls are for _generated_ or _server-derived_ UIDs, not tool inputs.
