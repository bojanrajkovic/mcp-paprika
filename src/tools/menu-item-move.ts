...
import { commitMenu, commitMenuItem, menuStartGuard } from "./menu-helpers.js";
...
export function registerMoveMenuItemTool(server: McpServer, ctx: ServerContext): void {
    ...
    async (args) => {
      ...
      return menuStartGuard(ctx).match(
        async (): Promise<CallToolResult> => {
          ...
          let saved: MenuItem;
          try {
            saved = (await ctx.client.saveMenuItems([moved]))[0]!;
