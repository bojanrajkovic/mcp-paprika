    ...
    async (args) => {
      ...
      return coldStartGuard(ctx).match(
        async (): Promise<CallToolResult> => {
          ...
          // wasted full-resolution encode) AND stash the full generated bytes
          // server-side under an opaque token, so the user can attach THIS exact
          // image later (`upload_photo` with `generation_token`) without
          // regenerating (non-deterministic) or round-tripping base64.
          if (!attach) {
              ...
              content: [
                {
                  ...
                  text:
                    ...
                    `Not attached. To save THIS exact image, call upload_photo with generation_token \`${token}\` ` +
