...
describe("update_menu_item tool", () => {
  it("returns sync-not-ready message when stores not loaded", async () => {
    ...
    const text = getText(await callTool("update_menu_item", { uid: "mi-1", type: { name: "Dinner" } }));
    expect(text.toLowerCase()).toContain("not yet synced");
  });
  ...
  it("merges a type change, preserving the recipe link, name, and day", async () => {
    ...
    await callTool("update_menu_item", { uid: "mi-1", type: { name: "Breakfast" } });
  ...
  it("reports a UID miss without saving", async () => {
    ...
    const text = getText(await callTool("update_menu_item", { uid: "ghost", type: { name: "Dinner" } }));
