...
/** Shared cold-start gate: recipe store synced (for recipe-reference checks) and category catalog hydrated. */
function categoriesReady(ctx: ServerContext): ReturnType<typeof textResult> | null {
  return coldStartGuard(ctx).match(
    () =>
      ctx.categoryStore.hasSynced ? null : textResult("The category catalog is still syncing; try again in a moment."),
    (guard) => guard,
  );
}
...
export function registerCreateCategoryTool(server: McpServer, ctx: ServerContext): void {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const gate = categoriesReady(ctx);
