...
- **`errorResult()` is the `isError` half of that channel — a schema-bearing tool returns it on every NON-success branch.** Once a tool declares `outputSchema`, the SDK's `validateToolOutput` requires valid `structuredContent` on every non-error result but skips an `isError` one. So a successful answer (including an empty one) is `toolResult(text, structured)` while a not-ready precondition gate / unknown UID / unparseable arg is `errorResult(text)` — otherwise the SDK eats the friendly message and substitutes its own generic schema error. The success/empty/`isError` contract and the gate exemption are pinned over the real transport in `src/kernel/tool.e2e.test.ts` (a unit test can't see it — `makeTestServer` discards the config and never validates).
