...
- `TombstoneEntityStore._tombstones` is `private` (not `protected`); concrete subclasses never need direct tombstone access, and `isTombstone()` is the only public surface
