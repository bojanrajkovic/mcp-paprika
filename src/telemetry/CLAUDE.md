...
- **Attribute discipline is stricter than the logs**: enum- or name-class values only (tool names, entity names, outcome enums). No UIDs, URIs, free text, emails, subs, or token material on any span or metric attribute — the `REDACT_PATHS` key list is the reference for what must never appear. The conformance test (`test/conformance/telemetry-attributes.test.ts`) gates the custom-attribute namespace.
