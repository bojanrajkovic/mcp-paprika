...
- **oxfmt formats Markdown too, and a double-star glob inside a bold lead breaks it.** A `**/*`-style glob literal placed inside a `**bold**` span unbalances oxfmt's emphasis parser and silently mangles the whole line (eaten spaces, `*`→`_`) — and oxfmt is _stable_ on the mangled output, so it commits clean and stays broken. When writing a `CLAUDE.md` / `.md` sharp edge, keep a glob literal in a code span in the bullet _body_, not the bold lead — or describe the path without the literal (e.g. "the `shared/` directory glob").
